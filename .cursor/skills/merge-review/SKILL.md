...
- 반영 위치(Cursor): `.cursor/rules/*.mdc`(또는 공통 `AGENTS.md`). 반복 절차가 정형화됐으면 `.cursor/commands/`의 명령으로, 머지 직후 자동 점검이 필요하면 `.cursor/hooks/`의 hook으로 승격한다.
...
- [ ] 불변 규칙을 하네스(`.cursor/rules/*.mdc`)에 고정(§5.4)
